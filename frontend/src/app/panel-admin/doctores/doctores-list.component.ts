        ...
        <table *ngIf="!isLoading && !errorMessage" class="w-full text-left text-sm text-slate-600">
          ...
          >
            <tr>
              ...
              <th *ngIf="isAdmin" scope="col" class="px-6 py-4 text-right">Acciones</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-50">
            ...
            <tr *ngIf="filteredDoctors.length === 0">
              <td [attr.colspan]="isAdmin ? 4 : 3" class="px-6 py-8 text-center text-slate-500">
      ...
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
        ...
        >
          <h3 class="font-bold text-lg text-slate-800">
            {{ isEditing ? 'Editar Doctor' : 'Añadir Doctor' }}
          </h3>
          <button
            (click)="closeModal()"
        ...
        <form (ngSubmit)="saveDoctor()" class="p-6 space-y-4">
          ...
          <div class="pt-4 flex gap-3">
            ...
            >
              {{ isSaving ? 'Guardando...' : (isEditing ? 'Guardar Cambios' : 'Guardar Doctor') }}
            </button>
          </div>
        </form>
      </div>
    ...
    >
      ...
      >
        ...
        <p class="text-sm text-slate-500 mb-6 leading-relaxed">
          Esta acción eliminará la cuenta del doctor permanentemente y revocará todo su acceso al sistema de forma irreversible.
        </p>
    ...
    if (this.isEditing && this.selectedId) {
      ...
      this.usersService.updateUser(this.selectedId, payload).subscribe({
        ...
        error: (err) => {
          ...
          this.errorMessage = err.error?.message || 'Error al actualizar el doctor';
          this.isSaving = false;
          this.cdr.detectChanges();
        },
      });
  ...
  confirmDelete() {
    ...
    this.usersService.deleteUser(this.idToDelete).subscribe({
      ...
      error: (err) => {
        ...
        this.errorMessage = 'No se pudo eliminar el doctor.';
        this.showDeleteConfirm = false;
        this.idToDelete = null;
        this.cdr.detectChanges();
      },
