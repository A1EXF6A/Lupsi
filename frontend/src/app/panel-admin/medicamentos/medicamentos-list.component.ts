...
@Component({
  ...
  template: `
    <div class="space-y-6">
      ...
      <!-- Búsqueda -->
      <div class="mb-4">
        <input type="text" [(ngModel)]="searchTerm" placeholder="Buscar medicamento..." class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500 shadow-sm">
      </div>
      ...
      <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <table *ngIf="!isLoading && !errorMessage" class="w-full text-left text-sm text-slate-600">
          ...
          <tbody class="divide-y divide-slate-50">
            <tr *ngFor="let item of filteredMedications | slice:0:visibleCount; let i = index" class="hover:bg-slate-50 transition-colors animate-fade-in-up" [style.animation-delay.ms]="i * 50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                ...
                <button (click)="deleteItem(item.id)" class="text-slate-400 hover:text-red-500 transition-colors mx-1" title="Eliminar">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M18 6 6 18" />
                    <path d="m6 6 12 12" />
                  </svg>
        ...
        <!-- Paginación local -->
        <div *ngIf="filteredMedications.length > visibleCount" class="p-6 border-t border-slate-100 flex justify-center">
    ...
    <div *ngIf="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <h3 class="font-bold text-lg text-slate-800">{{ isEditing ? 'Editar Medicamento' : 'Nuevo Medicamento' }}</h3>
...
export class MedicamentosListComponent implements OnInit {
  ...
  deleteItem(id: string) {
    if (confirm('¿Eliminar medicamento?')) {
      this.medicationsService.delete(id).subscribe({
        next: () => this.loadData(),
