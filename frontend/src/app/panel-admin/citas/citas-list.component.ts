...
@Component({
  ...
  template: `
    <div class="space-y-6">
      ...
      <!-- Búsqueda -->
      <div class="mb-4">
        <input type="text" [(ngModel)]="searchTerm" placeholder="Buscar por nombre de paciente, doctor o DNI..." class="w-full px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500 shadow-sm">
      </div>
        ...
        <table *ngIf="!isLoading && !errorMessage" class="w-full text-left text-sm text-slate-600">
          ...
          <tbody class="divide-y divide-slate-50">
            <tr *ngFor="let app of filteredAppointments | slice:0:visibleCount; let i = index" class="hover:bg-slate-50 transition-colors animate-fade-in-up" [style.animation-delay.ms]="i * 50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-center">
                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold"
                  [ngClass]="app.paid ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'">
                  {{ app.paid ? 'Pagado' : 'Pendiente' }}
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button *ngIf="app.status === 'COMPLETED'" (click)="openPrescriptionModal(app)" class="text-slate-400 hover:text-green-600 transition-colors mx-1" title="Ver Receta">
                  <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <div *ngIf="showPrescriptionModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
      <div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div class="p-6 overflow-y-auto">
          <div *ngIf="isLoadingPrescription" class="py-8 flex flex-col items-center justify-center text-slate-400">
            <svg class="animate-spin h-8 w-8 text-green-500 mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
              <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
          ...
          <div *ngIf="!isLoadingPrescription && prescriptions.length === 0" class="py-8 text-center text-slate-500 font-medium">
            No se ha emitido ninguna receta para esta cita.
          </div>

          <div *ngIf="!isLoadingPrescription && prescriptions.length > 0" class="space-y-4">
            <div *ngFor="let pres of prescriptions" class="p-4 border border-slate-200 rounded-xl bg-slate-50">
              <div class="font-bold text-slate-800 mb-1">{{ pres.medications?.name }}</div>
              <div class="text-sm text-slate-600 mb-2"><strong>Dosis:</strong> {{ pres.dosage }}</div>
              <div class="text-sm text-slate-600"><strong>Frecuencia:</strong> {{ pres.frequency }}</div>
              <div class="text-sm text-slate-600"><strong>Duración:</strong> {{ pres.duration }}</div>
              <div *ngIf="pres.notes" class="text-xs text-slate-500 mt-3 pt-3 border-t border-slate-200">
                <em>{{ pres.notes }}</em>
              </div>
            </div>
          </div>
          ...
          <div class="pt-6">
            <button (click)="closePrescriptionModal()" class="w-full py-2.5 bg-slate-100 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-200 transition-colors">Cerrar</button>
          </div>
        </div>
      </div>
    </div>
