      ...
      <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <table *ngIf="!isLoading && !errorMessage" class="w-full text-left text-sm text-slate-600">
          ...
          <tbody class="divide-y divide-slate-50">
            <tr
              *ngFor="let item of filteredOffices | slice: 0 : visibleCount; let i = index"
        ...
        <!-- Paginación local -->
        <div
          *ngIf="filteredOffices.length > visibleCount"
...
export class OficinasListComponent implements OnInit {
  ...
  get filteredOffices() {
    ...
    return this.offices.filter(
      (o) =>
        o.name.toLowerCase().includes(term) || (o.floor && o.floor.toLowerCase().includes(term)),
    );
  }
  ...
  confirmDelete() {
    ...
    this.catalogsService.deleteOffice(this.idToDelete).subscribe({
      next: () => {
        this.showDeleteConfirm = false;
        this.idToDelete = null;
        this.loadData();
