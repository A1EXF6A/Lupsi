  ...
  template: `
    <div class="p-4 border rounded shadow-sm bg-white">
      ...
      <div class="mb-4">
        ...
        <select [(ngModel)]="selectedDoctor" (ngModelChange)="loadSlots()" class="mt-1 block w-full p-2 border border-gray-300 rounded">
          <option [ngValue]="null">-- Seleccione --</option>
          <option *ngFor="let doc of doctors" [ngValue]="doc.id">
            Dr/Dra. {{ doc.profiles?.first_name || '?' }} {{ doc.profiles?.last_name || '?' }} - {{ doc.specialty || 'Sin especialidad' }}
          </option>
        </select>
      </div>
      ...
      <div class="mb-4">
        ...
        <input type="date" [(ngModel)]="selectedDate" (ngModelChange)="loadSlots()" class="mt-1 block w-full p-2 border border-gray-300 rounded">
      </div>
      ...
      <div class="mt-6">
        <button [disabled]="!selectedSlot" (click)="confirmBooking()" class="w-full bg-blue-600 text-white p-3 rounded font-bold disabled:bg-gray-400">
          Confirmar Reserva
        </button>
      </div>
    </div>
...
export class ReservaComponent implements OnInit {
  ...
  loadSlots() {
    console.log(`[ReservaComponent] loading slots for Dr: ${this.selectedDoctor}, Date: ${this.selectedDate}`);
  ...
  confirmBooking() {
    ...
    this.appointmentsService.createAppointment(payload).subscribe({
      ...
      error: (err) => {
        console.error(err);
        this.errorMessage = err.error?.message || 'Error al confirmar la reserva. El horario podría no estar disponible.';
        this.successMessage = '';
