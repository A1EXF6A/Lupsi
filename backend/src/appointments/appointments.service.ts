...
export class AppointmentsService {
  ...
  async create(patientId: string, createAppointmentDto: CreateAppointmentDto) {
    ...
    if (error) {
      if (error.code === '23P01') { // exclusion_violation (Double-booking constraint)
        throw new ConflictException('El horario seleccionado ya se encuentra ocupado por otra cita.');
      }
      throw new InternalServerErrorException(`Error al crear cita: ${error.message}`);
    }

    return data;
  }

  async findAll(userId: string, role: string, date?: string) {
    const supabase = this.supabaseService.getClient();
    ...
    if (error) {
        throw new InternalServerErrorException(`Error fetching appointments: ${error.message}`);
    }
    ...
    const { data: patientsData } = await supabase.from('patients').select('id, dni').in('id', patientIds);
    const { data: doctorsData } = await supabase.from('doctors').select('id, specialty').in('id', doctorIds);
    const { data: profilesData } = await supabase.from('profiles').select('id, first_name, last_name').in('id', [...patientIds, ...doctorIds]);

    return data.map(app => {
      const pProfile = profilesData?.find(p => p.id === app.patient_id);
      const patient = patientsData?.find(p => p.id === app.patient_id);
      
      const dProfile = profilesData?.find(p => p.id === app.doctor_id);
  ...
  async getAvailableSlots(doctorId: string, date: string) {
    ...
    if (error) {
      throw new InternalServerErrorException(`Error fetching slots: ${error.message}`);
    }
    ...
    for (let hour = 8; hour < 17; hour++) {
      for (let minute of [0, 30]) {
        ...
        const isOccupied = (appointments || []).some(app => {
          ...
          const appEnd = new Date(app.appointment_end_time);
